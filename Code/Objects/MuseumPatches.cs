...
internal class MuseumPatches
{
    ...
    /// <summary>
    /// Patches <c>LibraryMuseum.HasDonatedArtifact</c> to handle Sunberry artifacts/minerals showing up in collections menu.
    /// </summary>
    [HarmonyPatch(typeof(LibraryMuseum), nameof(LibraryMuseum.HasDonatedArtifact))]
	[HarmonyPrefix]
	public static bool HasDonatedArtifact_Prefix(string itemId, ref bool __result)
	{
		if (Artifacts1.Contains(itemId) && Game1.player.team.completedSpecialOrders.Contains("SBV.SpecialOrder.SBVMuseumArtifacts1"))
		{
			__result = true;
			return false;
		}
		
		if (Artifacts2.Contains(itemId) && Game1.player.team.completedSpecialOrders.Contains("SBV.SpecialOrder.SBVMuseumArtifacts2"))
		{
            __result = true;
            return false;
        }
		
		if (Minerals.Contains(itemId) && Game1.player.team.completedSpecialOrders.Contains("SBV.SpecialOrder.SBVMuseumMinerals"))
        {
            __result = true;
            return false;
        }
		
		if (itemId.Equals(LunarBean) && (Game1.player.mailReceived.Contains("skellady.SBVCP_LunarBeanGrown") || (Game1.player.basicShipped.TryGetValue("skellady.SBVCP_LunarBean", out int timesShipped) && timesShipped > 0) || (Game1.player.mineralsFound.TryGetValue("skellady.SBVCP_LunarBean", out int timesFound) && timesFound > 0)))
		{
            __result = true;
            return false;
        }
